...
#'   \item{estimateR2}{The R\eqn{^2} statistic and confidence interval in APA style for markdown printing.}
...
d.to.r <- function (d, n1, n2, a = .05) {
  ...
  output = list("r" = r, #r stats
                ...
                "F" = Fvalue,
                "p" = p,
                "estimate" = paste("$r$ = ", apa(r,2,FALSE), ", ", (1-a)*100, "\\% CI [",
                                   apa(rlow,2,FALSE), ", ", apa(rhigh,2,FALSE), "]", sep = ""),
                "estimateR2" = paste("$R^2$ = ", apa(rsq,2,FALSE), ", ", (1-a)*100, "\\% CI [",
                                   apa(limits$Lower.Conf.Limit.R2,2,FALSE), ", ",
                                   apa(limits$Upper.Conf.Limit.R2,2,FALSE), "]", sep = ""),
